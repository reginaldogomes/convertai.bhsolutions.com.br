...
describe('Next.js configuration compatibility', () => {
    ...
    it('runs the config preflight inline and via npm lifecycle hooks', () => {
        ...
        expect(packageJson.scripts.predev).toBe('node scripts/assert-next-config.mjs')
        expect(packageJson.scripts.prebuild).toBe('node scripts/assert-next-config.mjs')
    })
})
